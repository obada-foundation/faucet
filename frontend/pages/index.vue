<template>
  <div>
    <v-stepper v-model="step" alt-labels class="bgdarkopacity">
      ...
      >
        <div class="txtdts text-center">
          <p>If you are a developer looking to test the functionality of OBADA network or setting up a node on testnet, you can acquire <b>test</b> OBD tokens through this faucet.</p>
        </div>
        <v-btn
          @click="step++"
          depressed
      ...
      <v-stepper-step
        ...
        editable
      >
        Add your OBADA <b>testnet</b> Address
        <v-tooltip
        top
        close-delay="2000"
        >
          ...
          <span>Please enter your OBADA testnet wallet address which we'll use to transfer your test tokens.<br> It should begin with "obada1".</span>
        </v-tooltip>
      </v-stepper-step>
      ...
      >
        ...
        >
          ...
          >
            <v-text-field
              v-model="address"
              label="OBADA Wallet Address"
              :hint="`Example: ${this.$config.defaultTestingAddress}`"
              required
              class="col-12"
              :rules="address_rules"
            />
    ...
    <v-alert
      ...
      outlined
    >
      <b>Done! Your requests tokens should have arrived at your provided address ({{ this.$config.currentAmountGiven }} {{ this.$config.minimalDenom }}).</b>
    </v-alert>
    <v-alert
      icon="mdi-shield-lock-outline"
      prominent
...
export default {
  data: () => {
    return {
      ...
      address_rules: [
        value => !!value||`Required.\n Example: ${this.config.defaultTestingAddress}`,
        value => /^(obada)1[a-z0-9]{38}$/.test(value)||'Invalid OBADA address format.'
      ],
    }
  },
  ...
  methods: {
    ...
    async handle_submit () {
      ...
      const response = await this.handle_fetch()

      this.loading = !this.loading

      // Check this case
      if(!response) return this.error_non_existing_address = true
    ...
    async handle_fetch () {
      try {
        const response = await this.$axios.post(
          `${this.$config.faucetServer}/credit`,
          {
            coins: [`${ this.$config.currentAmountGiven }${ this.$config.minimalDenom }`],
